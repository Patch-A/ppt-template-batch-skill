...
def parse_page(base_url: str, html: str) -> tuple[list[AssetCandidate], list[AssetCandidate], list[str]]:
    ...
    for item in parser.link_icons:
        logo_candidates.append(AssetCandidate(src=urljoin(base_url, item["src"]), page=base_url, kind=item["kind"]))
    for item in parser.images:
        full = urljoin(base_url, item["src"])
        target = " ".join([item.get("src", ""), item.get("alt", ""), item.get("class", "")]).lower()
        if any(hint in target for hint in LOGO_HINTS):
            logo_candidates.append(
                AssetCandidate(src=full, page=base_url, kind=item["kind"], alt=item.get("alt", ""), cls=item.get("class", ""))
            )
        else:
    ...
    for item in parser.meta_images:
        visual_candidates.insert(0, AssetCandidate(src=urljoin(base_url, item["src"]), page=base_url, kind=item["kind"]))

    page_urls = pick_candidate_page_urls(base_url, parser)
    return logo_candidates, visual_candidates, page_urls


def search_candidate_pages(domain: str, buyer_name: str) -> list[str]:
    queries = [
    ...
    for query in queries:
        ...
        pages.extend(extract_search_links(final_url, html, domain))
    return dedupe(pages)[:MAX_PAGE_CANDIDATES]
...
def extract_search_links(base_url: str, html: str, domain: str) -> list[str]:
    ...
    for item in parser.links:
        href = item["href"]
        full = urljoin(base_url, href)
        if domain.lower() in urlparse(full).netloc.lower():
            candidates.append(full)
    return candidates
...
def discover_assets_for_domain(domain: str, buyer_name: str) -> tuple[str | None, list[AssetCandidate], list[AssetCandidate], list[str]]:
    ...
    for base_url in candidate_base_urls(domain):
        ...
        for page_url in dedupe(page_urls + search_pages):
            try:
...
def process_buyer(buyer: dict[str, Any], assets_dir: Path, cache: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for candidate in logo_candidates[:8]:
        ...
        trace.append(f"logo_try:{candidate.src}:score={candidate.score}")
        if downloaded:
    ...
    for candidate in visual_candidates[:10]:
        downloaded, meta = download_asset(candidate, assets_dir / f"{slug}-site", "site")
        trace.append(f"site_try:{candidate.src}:score={candidate.score}")
        if downloaded:
...
def main() -> int:
    ...
    report_items = []
    for item in buyers:
        buyer, report = process_buyer(dict(item), assets_dir, cache)
